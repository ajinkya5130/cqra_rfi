...
class RFIQuestionScrollViewModel:ViewModel() {
    ...
    private suspend fun getQuestionAnswerUsingRfId(qId: String): AnswerTableModel {
        val value = CustomTitle.rfiDB.answerDao().getAnswerUsingRfId(qId,selectedrfiId)
        Log.d(TAG, "getQuestionAnswer: value: $value")
        return value
    }
    ...
    fun getQuestionAnswerUsingRfIdAsync(
        qId: String,
        callback: (AnswerTableModel?) -> Unit
    ) {
        CoroutineScope(Dispatchers.IO).launch {
            try {
                val value = getQuestionAnswerUsingRfId(qId)
                withContext(Dispatchers.Main) {
                    if (value.PK_Answer_id == 0){
                        Log.d(TAG, "getQuestionAnswerUsingRfIdAsync: No data present = $value")
                        val answerTableModel = AnswerTableModel()
                        callback(answerTableModel) // Pass the result to Java
                    }else{
                        Log.d(TAG, "getQuestionAnswerUsingRfIdAsync: data present, value: $value")
                        callback(value) // Pass the result to Java
                    }
                }
            } catch (e: Exception) {
                withContext(Dispatchers.Main) {
                    Log.e(TAG, "getQuestionAnswerUsingRfIdAsync:Exception: ",e)
                    callback(null) // Handle error case
                }
            }
        }
    ...
    fun updateQuestionAnswer(value: AnswerTableModel) {
        viewModelScope.launch(Dispatchers.IO) {
            try {
                val updatedVal = CustomTitle.rfiDB.answerDao().update(value)
                Log.d(TAG, "insertDefaultAnswer: value: $updatedVal")
            }catch (e:Exception){
                Log.e(TAG, "insertDefaultAnswer: Exception: ",e)
            }
        }
